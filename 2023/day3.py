...
INPUT = """...317..........214.....................................751.................................630...479..205....41.993............416.........
...
for y, line in enumerate(lines):
    ...
    for x, c in enumerate(line):
        x_min = max(0, x - 1)
        x_max = min(len(line), x + 1)
        y_min = max(0, y - 1)
        y_max = min(len(lines) - 1, y + 1)
        neighbors = set(
            lines[y_min][x_min : x_max + 1]
            + lines[y][x_min : x_max + 1]
            + lines[y_max][x_min : x_max + 1]
        )
        if c.isdigit():
            is_part = is_part or any(map(is_symbol, neighbors))
            num += c
            part_id = part_id or (x, y)
            pos_to_part[(x, y)] = part_id
        else:
            pos_to_part[(x, y)] = None
        if not c.isdigit() or x == len(line) - 1:
            if is_part and num:
                parts[part_id] = int(num)
            num = ""
            is_part = False
            part_id = None
        ...
        if c == "*":
            ...
            def unique_parts(y_):
                return {
                    ...
                    if (p := pos_to_part[(x, y_)]) is not None
                }

            neighbors |= unique_parts(y_min)
            neighbors |= unique_parts(y)
            neighbors |= unique_parts(y_max)

            if len(neighbors) == 2:
                ratio = math.prod(map(lambda n: parts[n], neighbors))
