...
fn part1() {
    ...
    let res: u32 = INPUT
        ...
        .filter_map(|line| {
            ...
            let mut subsets = r.split(";").map(|s_sub| {
                let mut subset = Subset {
                    red: 0,
                    green: 0,
                    blue: 0,
                };
                for s_col in s_sub.split(",") {
                    match s_col.trim().split_once(" ").unwrap() {
                        (n, "red") => subset.red = n.parse().unwrap(),
                        (n, "green") => subset.green = n.parse().unwrap(),
                        (n, "blue") => subset.blue = n.parse().unwrap(),
                        _ => panic!(),
                    }
                }
                subset
            });
            if subsets.all(|subset| {
                subset.red <= bag.red && subset.green <= bag.green && subset.blue <= bag.blue
            }) {
                Some(Game {
                    id: id.parse().unwrap(),
...
fn part2() {
    let res: u32 = INPUT
        ...
        .map(|line| {
            ...
            for s_sub in r.split(";") {
                for s_col in s_sub.split(",") {
                    match s_col.trim().split_once(" ").unwrap() {
                        (n, "red") => min_subset.red = min_subset.red.max(n.parse().unwrap()),
                        (n, "green") => min_subset.green = min_subset.green.max(n.parse().unwrap()),
                        (n, "blue") => min_subset.blue = min_subset.blue.max(n.parse().unwrap()),
                        _ => panic!(),
                    }
                }
            }
...
const INPUT: &str = "Game 1: 14 green, 8 blue, 9 red; 5 blue, 4 green, 2 red; 4 red, 4 blue, 4 green; 1 blue, 3 green, 2 red; 10 red, 3 blue, 15 green; 2 red, 6 green, 3 blue
